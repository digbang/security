...
namespace spec\Digbang\Security\Throttling;
...
class DefaultDoctrineThrottleRepositorySpec extends ObjectBehavior
{
    ...
    public function it_should_implement_sentinels_throttle_repository_interface()
    {
        $this->shouldHaveType(ThrottleRepositoryInterface::class);
        $this->shouldHaveType(ThrottleRepository::class);
    }

    public function it_should_give_me_zero_for_empty_cases_of_global_delay(QueryBuilder $queryBuilder, AbstractQuery $query)
    {
    ...
    public function it_should_give_me_zero_for_empty_cases_of_ip_delay(AbstractQuery $query)
    {
        $query->getResult()->willReturn([]);
    ...
    public function it_should_give_me_zero_for_empty_cases_of_user_delay(AbstractQuery $query, User $user)
    {
    ...
    public function it_should_log_throttling_entries(EntityManager $entityManager)
    {
    ...
    public function it_should_log_throttling_ip_entries(EntityManager $entityManager)
    {
        $entityManager->persist(Argument::type(DefaultThrottle::class))
    ...
    public function it_should_log_throttling_user_entries(User $user, EntityManager $entityManager)
    {
        $entityManager->persist(Argument::type(DefaultThrottle::class))
            ->shouldBeCalledTimes(2);
    ...
    public function it_should_log_throttling_user_and_ip_entries(User $user, EntityManager $entityManager)
    {
        $entityManager->persist(Argument::type(DefaultThrottle::class))
    ...
    public function it_should_give_me_the_registered_global_delay_when_it_is_set(QueryBuilder $queryBuilder, AbstractQuery $query, DefaultGlobalThrottle $globalThrottle)
    {
    ...
    public function it_should_give_me_the_registered_ip_delay_when_it_is_set(QueryBuilder $queryBuilder, AbstractQuery $query, DefaultIpThrottle $ipThrottle)
    {
        Carbon::setTestNow($now = Carbon::now());
